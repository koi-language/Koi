...
class KoiDefinitionProvider {
  ...
  getContext(line, word) {
    ...
    if (/uses\s+Skill\s+[\w\s,]+/.test(line)) {
      // Check if word is one of the skill names (comma-separated)
      const usesSkillMatch = line.match(/uses\s+Skill\s+([\w\s,]+)/);
      if (usesSkillMatch) {
    ...
    if (/uses\s+Team\s+[\w\s,]+/.test(line)) {
      // Check if word is one of the team names (comma-separated)
      const usesTeamMatch = line.match(/uses\s+Team\s+([\w\s,]+)/);
      if (usesTeamMatch) {
    ...
    if (/Agent\s+\w+\s*:\s*\w+/.test(line) && line.includes(word)) {
      return { type: 'Role' };
    }

    // Team <name> { member = <word> } (agent reference)
  ...
  searchInDocument(document, symbolName, symbolType) {
    ...
    switch (symbolType) {
      case 'Skill':
        pattern = new RegExp(`^\\s*Skill\\s+${symbolName}\\s*\\{`, 'm');
        break;
      case 'Agent':
        pattern = new RegExp(`^\\s*Agent\\s+${symbolName}\\s*:`, 'm');
        break;
      case 'Role':
        pattern = new RegExp(`^\\s*role\\s+${symbolName}\\s*\\{`, 'm');
        break;
      case 'Team':
        pattern = new RegExp(`^\\s*Team\\s+${symbolName}\\s*\\{`, 'm');
        break;
      default:
        return null;
    }
