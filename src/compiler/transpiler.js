...
export class KoiTranspiler {
  ...
  generatePromptDecl(node) {
    ...
    if (node.content.type === 'ComposeDecl') {
      const { fragments, rules, model } = node.content;
      ...
      if (resolverCode) {
        ...
        const indentedResolver = resolverCode
          ...
          .join('\n');
        return this.emit(
          `const ${name} = { __isCompose__: true, fragments: {\n${fragmentsStr}\n  }, resolve: async (fragments, callAction) => {\n${indentedResolver}\n  } };\n\n`,
          node
        );
      }

      // Fallback: static object with rules (runtime LLM will resolve at execution time)
      const rulesStr = JSON.stringify(rules);
      const modelStr = model ? `, model: ${JSON.stringify(model)}` : '';
      return this.emit(
        `const ${name} = { __isCompose__: true, fragments: {\n${fragmentsStr}\n  }, rules: ${rulesStr}${modelStr} };\n\n`,
        node
      );
    }
    ...
    if (node.params.length > 0) {
      ...
      const content = node.content.value.replace(/`/g, '\\`');
  ...
  generateEventHandler(node, agentName = null) {
    ...
    if (hasOnlyPlaybook) {
      ...
      if (isSingleStringPart) {
        ...
        code += this.emit(`${this.getIndent()}    if (__p__ && __p__.__isCompose__) return await __agent__._executeComposePrompt(__p__);\n`);
