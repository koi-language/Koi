...
export function renderDiffLine(type, lineNum, text, filePath) {
  ...
  // Colors matching Claude Code:
  // - Removed: dark red bg (48;5;52), light text (38;5;210 for default)
  // - Added: dark green bg (48;5;22), light text (38;5;157 for default)
...
export function renderColoredDiff(hunks, filePath) {
  const output = [];
  output.push(`\x1b[1m  ${filePath}\x1b[0m`);
...
export function renderContentDiff(oldContent, newContent, filePath) {
  ...
  if (m * n > 5000 * 5000) {
    const output = [`\x1b[1m  ${filePath}\x1b[0m`];
...
export function renderNewFileDiff(content, filePath) {
  ...
  const output = [`\x1b[1m  ${filePath} (new file)\x1b[0m`];
  for (let i = 0; i < lines.length; i++) {
    output.push(renderDiffLine('add', i + 1, lines[i], filePath));
  }
  return output.join('\n');
}
