...
class CLILogger {
  ...
  setInfo(slotOrText, text) {
    if (text !== undefined) {
      // Named slot mode: setInfo('tokens', '↑3.2k')
      // Skip the 'model' slot when model display is disabled
      ...
      // Legacy single-string mode: setInfo('full text')
      this._infoSlots._default = slotOrText;
    }

    // Combine slots: model + tokens are joined with a space (no · between them),
    // all other slots are joined with ' · '.
    const modelVal  = this._infoSlots['model'];
    const tokensVal = this._infoSlots['tokens'];
    const restVals  = Object.entries(this._infoSlots)
      .filter(([k]) => k !== 'model' && k !== 'tokens')
      .map(([, v]) => v)
      .filter(Boolean);
    const parts = [];
    if (modelVal && tokensVal) parts.push(`${modelVal} ${tokensVal}`);
    else if (modelVal)  parts.push(modelVal);
    else if (tokensVal) parts.push(tokensVal);
    parts.push(...restVals);
   ...
   * Delegates to provider.thinking() if set.
   */
  planning(prefix) {
   ...
   * Delegates to provider.stopThinking() if set.
   */
  stopAnimation() {
   ...
   * Delegates to provider.clear() if set.
   */
  clearProgress() {
    if (_provider?.clear) {
      _provider.clear();
