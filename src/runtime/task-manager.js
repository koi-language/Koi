 ...
 * Notifies the CLI UI via cliLogger.setTaskPanel() on every change.
 *
...
class TaskManager {
  ...
  _loadIfNeeded() {
    ...
    try {
      ...
      if (data && data.tasks) {
        ...
        // NOTE: Do NOT call cliLogger.setTaskPanel() here.
  ...
  _save() {
    ...
    cliLogger.setTaskPanel(this._tasksForUI());
  }
  ...
  update(id, updates = {}) {
    ...
    if (updates.status === 'completed') {
      ...
      if (nonDeleted.length > 0 && nonDeleted.every(t => t.status === 'completed')) {
        setTimeout(() => {
          cliLogger.setTaskPanel([]);
          cliLogger.print('\x1b[32m✓ All tasks completed.\x1b[0m');
        }, 300);
      }
    }
    return { ...task };
